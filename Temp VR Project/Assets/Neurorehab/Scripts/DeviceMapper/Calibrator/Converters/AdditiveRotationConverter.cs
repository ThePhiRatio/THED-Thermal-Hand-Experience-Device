...
{
    [Serializable]
    public class AdditiveRotationConverter : MonoBehaviour, IAdditiveConverter, ISerializationCallbackReceiver
    {
        ...
        public float GetValue(float valueRead, float currentValue, AxisLabels axis)
        {
            ...
            var multiplier = GetTForLerp(calibratedValue, Tolerance[axis] / 100f / 2f);

            if (calibratedValue < .5)
                multiplier *= -1;

            return multiplier * MaxSpeed[axis] * Time.fixedDeltaTime + currentValue;
        }
        ...
        private float GetTForLerp(float value, float tolerance)
        {
            ...
            if (value <= .5 + tolerance && value >= .5 - tolerance) // close to center, don't move. % tolerance
                return 0;
            //somar a tolerancia para o outro ramo
